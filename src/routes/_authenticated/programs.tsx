...
function ProgramsPage() {
  ...
  async function handleDeleteAllPrograms() {
    ...
    setDeletingAll(true);
    try {
      const { data: programs, error: readError } = await supabase.from("programs").select("id");
      if (readError) throw readError;
      const ids = (programs ?? []).map((item) => String(item.id));
      if (ids.length) {
        ...
        const paths = (evidence ?? []).map((item) => String(item.file_path ?? "")).filter(Boolean);
        if (paths.length) {
          const { error: storageError } = await supabase.storage.from("evidences").remove(paths);
          if (storageError) throw storageError;
        }
        if ((evidence ?? []).length) {
          const { error: evidenceDeleteError } = await supabase.from("evidences").delete().in("linked_ref", ids);
      ...
      toast.success("تم حذف كافة البرامج ومرفقاتها بنجاح");
    } catch (e) {
