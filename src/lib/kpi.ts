...
export function computeKpis(input: KpiInput): Kpi[] {
  const { planTasks, cases, attendance, interviews, students } = input;
  ...
  const attendanceRate = students.length ? Math.max(0, 100 - Math.round((absences / (students.length * 20)) * 100)) : 0;
